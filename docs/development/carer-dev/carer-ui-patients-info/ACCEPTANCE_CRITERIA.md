...
| AC-03 | US-01 | happy | Given a card for Margaret, when clicked, then the patient page for Margaret opens (`/carer/patients/client-margaret`, which goes to the Info tab). | MET |
...
| AC-07 | US-01 | happy | Given a patient page, when it renders, then it shows a 'Back to patients' link to `/carer/patients`, the name 'Margaret' with '78 years · Preston VIC', and tabs Home, Calendar, Info, Care log linking to `/carer/patients/client-margaret/{home,calendar,info,tasks}`, the current tab marked `aria-current="page"`. | MET |
