...
Test results: 40/40 feature cases pass (after CHG-024). Full suite 1277 pass, 5 fail (F0-07/F0-04 integration, Supabase `Invalid API key`, environment). Lint 0 errors, tsc clean, build passes. e2e 30/30 on three runs (one flaky calendar failure on the first run). Sweep clean.
...
Important decisions: FD-08 (a visually hidden `h2` between the `h1` and the kit's `h3` card titles); CHG-024/FD-09 (read-only until 'Edit'; no Cancel).
