...
- Decision (the parts PD-060 leaves to the build, chosen here as defaults and flagged for review in the PR):
  ...
  - **As built (`36131f8`).**
    - The dialog is drawn inline, not in a portal, so axe checks it. Close is its only control, so Tab and Shift+Tab stay on Close.
    - The button's text is a span that is cut at two lines, and the full description sits in that span's `title`, as FD-09 asks of the visible text.
    - The Pending costs table follows History's grid (FD-03, FD-10). Narrow: the description, then the bucket, then the date and the amount on one line. Wide: DATE, BUCKET, DESCRIPTION and AMOUNT, with the amount track kept at `clamp(9rem, 18%, 12rem)`.
    - The skeleton has three cards.
