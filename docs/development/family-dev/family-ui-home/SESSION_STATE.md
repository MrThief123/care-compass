...
- M3 done (tasks 2, 3, 4): loader reads page one of `done` and `overdue` (never the unfiltered log), Overdue card total + "View all N overdue", link rows (`activity-link-row.tsx`, replaces `recent-activity-row.tsx`), cards are server components (no router). Existing tests changed: `answerTaskLog` helper now filters by status; Scope call assertion `(CLIENT_ID)` -> `(CLIENT_ID, { status: "done" })`; the two chevron tests (button + `router.push`) -> link `href` tests. Record all in DECISIONS.
- ADDENDUM (human, 2026-09-20): nothing may overlap at any width; clean 1920 to 1024, graceful to 768, info-only at 640 and 375; real-browser width sweep; breakpoints recorded in DECISIONS, sweep table in PROGRESS. Plan: outer grid, one column below `xl` (1280), `md:grid-cols-2` for Overdue and Recent, `xl:grid-cols-[minmax(0,1fr)_340px]`; budget grid `auto-fit minmax`.
- Next step: task 5 Budget strip (local bucket card), responsive grid in `family-home-view.tsx`, then docs, then Playwright sweep.
