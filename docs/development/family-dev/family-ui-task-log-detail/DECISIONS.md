...
### FD-25 — The search box and the Status select share one top and bottom edge (the human's review, 2026-09-20). **HUMAN REVIEW: deviates from the design**
- Date: 2026-09-20
- Context: the human reviewing the built Task log: "search bar and status should be aligned with each other, currently status is a bit too low." Reproduced in Chromium at 1440, 1024 and 768: the search box spanned y 170 to 214 and the select 198 to 242, 28px lower. Cause: the kit `Field` draws its label ("Status", 20px line) and an 8px gap (FD-23 item 3) above the select, and the row aligned the two fields by their top edges, so the select sat one label lower. `docs/design/screens/family-07-task-log.png` draws it that way too; the human's instruction outranks the mockup.
- Decision: in `task-log-view.tsx` the search `<form>` gets `FILTER_ROW_SEARCH`: `text-body-default @min-[30.5rem]:mt-[calc(1lh+0.5rem)]`. `text-body-default` is the label's type token, so `1lh` is the label's line height (20px) and `0.5rem` is the field's `gap-2`; the offset therefore follows the label if either token changes. The row is `@container flex flex-wrap items-start gap-3`. `30.5rem` is the row width at which the two fields share a line: 16rem (the search's flex basis) + 12px (`gap-3`) + 220px (the select). Narrower than that they stack and the search box has no offset (no gap above it, as before). Measured after: 170/214/44 (top/bottom/height) for both at 1440, 1024 and 768; stacked at 480 with 20px between the "Task log" heading and the search box (asserted by the e2e spec), and stacked, not overlapping, at 320 (screenshot).
- Reason: the offset moves the box, not the row's alignment, so the "No matches for ..." line under the search box (which makes the form taller than its box) cannot pull the two out of line. It is a container query, not a window media query, so it stays right if the row's width changes for another reason (the rail, the page padding).
- Alternatives considered: `items-end` on the row (rejected: aligns the bottoms of the two fields, and the "No matches" line makes the search form taller, so the search box would jump up whenever that message shows; reasoned from the layout, not tried); a fixed `mt-7` (rejected: 28px is a copy of two other numbers and drifts if either changes); moving the "Status" label beside the select or hiding it (rejected: changes the kit `Field` and drops a visible label; lane S owns both); a window breakpoint (rejected: the row width is what decides whether the fields share a line).
- Consequences: the `30.5rem` threshold is coupled to the search's `16rem` basis, the `gap-3` and the select's `220px`; changing any of them means changing the threshold (the class test and the e2e spec pin both sides). The design owner should confirm that aligning the boxes, not the label baselines, is what is wanted; the kit request in FD-23 (`Field` label gap) is unchanged.
- Human confirmation required: no for the change (requested by the human, 2026-09-20); yes for the design owner to note the deviation from `family-07-task-log.png`.
- Test changes caused: none to existing tests. New: `tests/e2e/family-task-log-filters.spec.ts` (3 real-browser specs: aligned at 1280 and 640 wide, also with the "No matches" line showing; stacked without a gap above the search at 480), red first in b02f371 and after the revision in 5619d5e (boxes 28px apart without the offset), and one jsdom test in `task-log-view.test.tsx` that pins the container query and the offset classes (red first, 5619d5e). Environment finding: `next dev` answers `/_next/static/chunks/*` with 403 to the `127.0.0.1` origin that `playwright.config.ts` uses (Next 16 `allowedDevOrigins`), so the client bundle never loads and typing does nothing in an e2e run; the spec therefore opens `?q=zzzz` (server-rendered) instead of typing. Any future e2e that needs client behaviour against `next dev` will hit the same 403; it is a one-line `allowedDevOrigins` change in `next.config.ts`, which is not this lane's file, so it is left for the human (see PROGRESS.md).

### FD-26 — The shared shell fix (sticky rail, wrapping header) is merged into this branch
- Date: 2026-09-20
- Context: the human's same review asked for (1) the rail to stay in view while a long screen scrolls and (2) no overlap or spill in the header row with the patient's details when the window is narrow, on all screens. That is a change to the shared shell (`Rail`, `PageHeader`) and to the family `layout.tsx`, so it was built and recorded as its own shared branch, `fix/shared-app-shell-nav-header`, as FD-04 of F0-15 (`docs/development/shared/shared-app-shell/DECISIONS.md`).
- Decision: this branch merges `origin/fix/shared-app-shell-nav-header` (b27c2e6, commits 4f4dad0, 197d428, 41fec21) so the previews and the sweeps here show the fix. No file of this feature's is changed by it; `src/components/shared/**`, the family `layout.tsx` and `tests/e2e/shared-app-shell.spec.ts` come in with it.
- Reason: same pattern as FD-14 (UI-04): a shared change is reviewed once, in its own PR, and reaches this branch through `main` and `family-dev`.
- Consequences: until the shell fix reaches `family-dev`, this PR's diff also shows those commits. PR order: UI-04 to `main`, then the shell fix to `main`, then `main` into `family-dev`, then this PR (nothing is opened without the human's yes, PD-056). Measured on this branch after the merge: page-level horizontal scroll is 0 from 1920 down to 320 on all five pages (it was +57px at 375 before, caused by the header, noted in FD-20 and FD-21).
