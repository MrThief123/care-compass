...
- Task detail links carry the validated q / status / page, and 'Back to Task log' returns to that exact view. Any task in the history can be opened, past or future (read with `getOccurrence`), and its Documents card lists the event's documents (`getEventDocuments`: name, type and size; metadata only, so tiles are not links) (CHG-005, confirmed by the human 2026-09-19).
