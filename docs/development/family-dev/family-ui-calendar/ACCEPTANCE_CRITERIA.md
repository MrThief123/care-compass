...
| AC-09 | US-01 | happy | Added by CHG-017. Given any view, when the calendar renders, then a primary "Enter event" link (44px tall) sits just left of D/W/M and opens Add event carrying the Calendar's origin (`from=calendar` with the current `view / date / month`, including a day picked in the range); on Add event, Save event (valid) and Cancel return to that same Calendar view. No horizontal overflow from 1920 to 768 px. | NOT MET |
