...
## Completed
- Contract gap found and closed under root **CHG-006** (human-approved, made on this branch, FD-01). Added `getOccurrences(clientId, { from, to })` and `getToday()` to the events contract, with mocks, `OccurrenceRangeSchema`, and the Tue 1 – Sat 5 Dec design rows, which are kept out of the Task log (FD-02).
...
- Shared (CHG-006): `src/server/events/queries.ts`, `src/server/events/occurrences.test.ts`, `src/mocks/queries/events.ts`, `src/mocks/fixtures.ts`, `src/types/domain.ts`
...
- The events contract had no range read (CHG-006).
- History rewrite and a bulk rename to renumber CHG-006 → CHG-007 were blocked by the session's permission policy, so the number stays CHG-006 for the human to settle.
