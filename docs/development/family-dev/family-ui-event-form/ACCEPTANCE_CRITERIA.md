...
| AC-10 | US-01 | happy / validation | Added by CHG-017. Given Add event opened from the Calendar (`from=calendar` with `view / date / month`), when Save event (valid) or Cancel is pressed, then it goes to that Calendar view. The Calendar params are re-validated by the Calendar's parser (bad values fall back to their defaults); any other or hostile `from` goes to Home as in AC-09. | MET |
