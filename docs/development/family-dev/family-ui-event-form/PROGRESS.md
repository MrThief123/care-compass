...
**CHG-017 (2026-09-24): PR OPEN** (#88 to `family-dev`, built on `feature/family-ui-calendar-add-event` with FAM-UI-02). Add event opened from the Calendar (`from=calendar` plus its view) returns there on Save event and Cancel; any other origin still goes Home. New AC-10 (MET), T-10 (PASS), FD-11. Files: `src/features/family-event-form/event-form-return.ts`, `src/app/(family)/family/[clientId]/events/new/page.tsx`, tests. Red first: 2977ff8. Checks: see FAM-UI-02 PROGRESS.md (CHG-017).
