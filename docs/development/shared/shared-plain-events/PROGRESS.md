...
- Merge conflicts with `main` (#78 CHG-009, #80 CHG-010 self-serve sign-up), resolved 2026-09-24 by merging `origin/main`: the stripe change was CHG-010 here and clashed with `main`'s CHG-010, so it is now **CHG-011** (DECISIONS.md, AC-12, FD-05, this file, `status-cue.ts`); DEVELOPMENT_PLAN totals are now 81 features / 341 criteria (UI-05 + F0-17); PRD REQ-35 keeps UI-05 in its features and REQ-36 comes from `main`. After the merge: typecheck clean, lint 0 errors (3 pre-existing warnings), `npm run test` 510 passed / 5 failed (the same environment-only integration failures).
