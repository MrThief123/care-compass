...
Current blocker: none. FD-02 ANSWERED: option a (the two dev-preview pages may show plain-event and switch examples). FD-01 ANSWERED: Option A (no `type` = tasks only); log and export callers pass `type: "all"`.
...
Important decisions: FD-01 (answered, Option A), FD-02 (answered, option a).
