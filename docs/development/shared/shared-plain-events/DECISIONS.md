...
- Bar: a solid neutral stripe (`bg-text-secondary`), per **CHG-011**. A dotted bar was tried first so a plain event would not differ from a Planned task by colour alone on compact day blocks, week blocks and month chips (where Planned also has no shape and the status word is read out only); the human preferred the solid stripe and accepted that trade-off for event type (status is still never colour alone).
