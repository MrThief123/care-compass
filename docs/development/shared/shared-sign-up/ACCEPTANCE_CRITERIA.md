...
| AC-02 | US-02 | happy | Given no account for owner@example.test, when they sign up as 'Organisation' with organisation name 'Wattle Care', then 'Wattle Care' exists, their profile is `admin` of it, and they are routed exactly as an admin sign-in is (F0-07 MFA gate, then `/admin/home`). | NOT MET |
