...
| AC-03 | US-02 | happy | Given the Carer rail, when rendered, then items are exactly Home, Patients, Calendar, Settings. | MET |
...
| AC-06 | US-02 | happy | Given Aisha on /carer/calendar, when the page loads, then the Calendar rail item is marked active (aria-current='page'). | MET (verified against `next dev`, not `next build` — see DECISIONS.md FD-03) |
