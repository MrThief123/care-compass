...
Current branch: n/a — `feature/shared-supabase-environment` not created
Worked on: n/a
What changed: n/a
Tests run: none
Test results: n/a
...
Exact next action: Run `START FEATURE F0-04` once dependencies are MERGED TO DEV (or COMPLETE) and blocking decisions are answered.
Files likely to be touched next: `supabase/config.toml`, `src/lib/env.ts`, `.env.example`, `src/lib/supabase/browser.ts`, `src/lib/supabase/server.ts`, `src/lib/supabase/middleware.ts`
