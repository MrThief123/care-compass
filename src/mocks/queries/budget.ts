 ...
 * the client's pending fund entries of that kind. Pending costs are not taken
...
export async function getBudgetSummary(clientId: string): Promise<BudgetBucketSummary[]> {
  ...
  return raw.map((bucket) => {
    const ofKind = pending.filter((entry) => entry.bucketKind === bucket.kind);
    const pendingCents = ofKind.reduce((sum, entry) => sum + Math.round(-entry.amount * 100), 0);
    return {
      ...deriveBudgetBucketSummary(bucket),
      pendingTotal: pendingCents / 100,
      pendingCount: ofKind.length,
    };
  });
}
