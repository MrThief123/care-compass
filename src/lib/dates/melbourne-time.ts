/**
 * Melbourne wall-clock helpers. Business logic is Australia/Melbourne (CLAUDE.md §7); these
 * convert between instants (ISO strings with an offset, which is how the database and the
 * fixtures store them) and the wall-clock local times the recurrence engine works in (F0-09).
 * Moved here from `src/mocks/melbourne-time.ts` by F0-11, which needs the same conversion for
 * real data; the mock layer re-exports it.
 */
...
/**
 * The Melbourne wall-clock time (`YYYY-MM-DDTHH:mm:ss`, no offset) of an instant written in any
 * offset, for example `2026-11-29T22:00:00Z` becomes `2026-11-30T09:00:00`. This is the form
 * `expandOccurrences` takes. In the hour the clocks go back, the two instants that share a
 * wall-clock time give the same result. Throws for something that is not a date-time.
 */
export function instantToMelbourneLocal(iso: string): string {
