...
describe("[FAM-UI-05][PRD] loadFamilyBudgetData", () => {
  it("[FAM-UI-05][AC-01] gathers the bucket summaries and the fund history, and nothing else", async () => {
    const data = await loadFamilyBudgetData(CLIENT_ID);

    expect(data).toEqual({ buckets: [BUCKET], history: [ENTRY] });
  });
  ...
  it("[FAM-UI-05][AC-03] passes an empty history and empty buckets through as they are", async () => {
    ...
    expect(await loadFamilyBudgetData(CLIENT_ID)).toEqual({ buckets: [], history: [] });
  });
